...
g_omegaomega <- function(y, xi, omega, alpha, nu) {
  ...
  (-1 / omega + z^2 * tau^2 / omega - alpha * z * tau * nu * w / (w * (nu + z ^ 2))) ^ 2 * dst(y, xi = xi, omega = omega, alpha = alpha, nu = nu)
}
...
g_xiomega <- function(y, xi, omega, alpha, nu) {
  ...
  (z * tau ^ 2 / omega - alpha * tau * nu * w / (omega * (nu + z ^ 2))) * (-1 / omega + z^2 * tau^2 / omega - alpha * z * tau * nu * w / (w * (nu + z ^ 2))) * dst(y, xi = xi, omega = omega, alpha = alpha, nu = nu)
}
...
g_omegaalpha <- function(y, xi, omega, alpha, nu) {
  ...
  (-1 / omega + z^2 * tau^2 / omega - alpha * z * tau * nu * w / (w * (nu + z ^ 2))) * (z * tau * w) * dst(y, xi, omega, alpha, nu)
}
...
g_omeganu <- Vectorize(function(y, xi, omega, alpha, nu) {
  ...
  (-1 / omega + z^2 * tau^2 / omega - alpha * z * tau * nu * w / (w * (nu + z ^ 2))) * (0.5 * (digamma(1+nu/2) - digamma(nu/2) - (2*nu + 1)/(nu * (nu + 1)) 
                                                                                  - log(1 + z^2 / nu) + z^2 * tau^2 / nu + (alpha * z * (z^2 - 1) * w)/((nu + z^2)^2 * tau) 
                                                                                  + sapply(y, function(y) { # Gamma
                                                                                    integrate(function(u, nu) gamma_integrand(u, nu), lower = -Inf, upper = varxi, nu = nu, rel.tol = 1e-12)$value
                                                                                  }) / pt(varxi, nu + 1))) * dst(y, xi, omega, alpha, nu)
...
stInfoMat <- function(xi, omega, alpha, nu){
