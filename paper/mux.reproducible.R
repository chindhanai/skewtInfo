...
# Figure 9
hugedof <- retReps[which(skewtFits[, 4] > 1000)[1], ]
dens <- density(hugedof, bw="nrd0")
plot(dens, ylab="Density", xlab="Skew-t Random Samples", main="Huge Dof Density Plot",
     col=3, lwd=1, las = 1, ylim = c(0, 6.2))
rug(hugedof, col = 6)
chart.QQPlot(hugedof, envelope = 0.95, distribution = "norm",
             distributionParameter = "mean = mean(hugedof),sd = sd(hugedof)", pch = 20,
             col = c(4,2), main = "Huge Dof Normal QQ Plot", xlab = "Normal Quantiles")
...
# Does produce the top and bottom figures in Figure 10
#####################################################################
for (i in 1:length(n)) {
